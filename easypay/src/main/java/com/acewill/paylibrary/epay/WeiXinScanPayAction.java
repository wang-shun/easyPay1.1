...
public class WeiXinScanPayAction {
	...
	public static EPayResult unifiedorder(String appId, String mchID,
		...
		try {
			...
			String time_expire = DateUtils.getDateToStringForWX(System
					.currentTimeMillis() + 40 * 1000); // 90秒超时时间
	...
	public static EPayResult micropay(String appId, String mchID, String key,
		...
		Log.e("myLog", "Wechat支付参数:" + ",appId:" + appId + ",mchID:" + mchID + ",key:" + key + ",authCode:" + authCode + ",out_tradeNo:" + out_tradeNo + ",fenAmount:" + fenAmount + ",body:" + body
				+ ",sub_mch_id:" + sub_mch_id + ",deviceInfo:" + deviceInfo);

		Log.e(TAG, "微信开始处理的时间:" + format.format(new Date()));
		EPayResult result = new EPayResult();
		try {
			System.out.println("key = " + key + ",appid = " + appId
					+ ",mch_id = " + mchID + ",sub_mch_id:" + sub_mch_id);

			WXPay.initSDKConfiguration(key, appId, mchID, sub_mch_id, "", "");
			// 90秒超时时间
			String time_expire = DateUtils.getDateToStringForWX(System
					.currentTimeMillis() + 40 * 1000);
			...
			synchronized (resBuf) {
				WXPay.doScanPayBusiness(data, new ResultListener() {
					...
					public void onFail(ScanPayResData arg0) {
						...
						Log.e(TAG, "onFail支付失败的时候回调的时间:" + format.format(new Date()));
						synchronized (resBuf) {
							resBuf[0].success = false;
							resBuf[0].desc = arg0.getErr_code_des();
							resBuf.notifyAll();
					...
					public void onSuccess(ScanPayResData arg0, String arg1) {
						System.out.println("onSuccess " + arg0 + " res = "
								+ arg1);
						Log.e(TAG, "微信处理完成的时间:" + format.format(new Date()));
						synchronized (resBuf) {
	...
	public static EPayResult query(String appId, String mchID, String key,
		...
		try {
			...
			ScanPayQueryReqData queryReq = new ScanPayQueryReqData(null,
					out_tradeNo);
			...
			PrintFile.printResult("result>>" + scanPayResData.getResult_code(), "wechatPayReturn");
