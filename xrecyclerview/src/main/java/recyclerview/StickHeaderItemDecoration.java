...
public class StickHeaderItemDecoration extends RecyclerView.ItemDecoration {
	...
	public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {
		...
		mListener.onDeDraw(parent.getChildCount());
		int     position     = 0;
	...
	protected boolean isHorizontal(RecyclerView parent) {
		...
		if (layoutManager instanceof LinearLayoutManager) {
			isHorizontal = ((LinearLayoutManager) layoutManager)
	...
	protected void calculateViewDrawRect(Rect outRect, RecyclerView parent, View headerView, boolean isHorizontal) {
		...
		if (isHorizontal) {
			...
			drawBottom = drawTop + (parent.getHeight() - parent.getPaddingBottom() - parent
					.getPaddingTop());
		} else {
			//竖向布局
			//宽填充整个parent
			//高根据view处理
			drawRight = drawLeft + (parent.getWidth() - parent.getPaddingLeft() - parent
					.getPaddingRight());
	...
	protected void measureHeaderView(RecyclerView parent, View headerView, boolean isHorizontal) {
		...
		if (!isHorizontal) {
			...
			widthSpec = View.MeasureSpec
					.makeMeasureSpec(parent.getWidth(), View.MeasureSpec.UNSPECIFIED);
			heightSpec = View.MeasureSpec
					.makeMeasureSpec(parent.getHeight(), View.MeasureSpec.EXACTLY);
		}

		//测量并计算headerView宽高
		int childWidth = ViewGroup.getChildMeasureSpec(widthSpec,
				parent.getPaddingLeft() + parent.getPaddingRight(), headerView
						.getLayoutParams().width);
		int childHeight = ViewGroup.getChildMeasureSpec(heightSpec,
				parent.getPaddingTop() + parent.getPaddingBottom(), headerView
